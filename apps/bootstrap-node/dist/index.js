...
class BootstrapIndexNode {
    ...
    HTTP_PORT = parseInt(process.env.HTTP_PORT || '3100');
    ...
    setupExpress() {
        ...
        this.app.post('/api/submit', async (req, res) => {
            try {
                ...
                await this.ipfs.pubsub.publish(this.PUBSUB_TOPIC, Buffer.from(JSON.stringify({
                    type: 'name-register',
                    record,
        ...
        this.app.post('/api/submit/v2', async (req, res) => {
            try {
                // Use official protocol deserialization
                const record = fromJSON(JSON.stringify(req.body));
    ...
    async subscribeToPubsub() {
        try {
            ...
            await this.ipfs.pubsub.subscribe(this.v2Manager.getPubsubTopic(), (msg) => {
    ...
    async publishIndex() {
        try {
            ...
            // Announce on pubsub
            await this.ipfs.pubsub.publish('frw/index/updates', Buffer.from(JSON.stringify({
                type: 'index-update',
                nodeId: this.nodeId,
                cid,
    ...
    async syncWithNetwork() {
        ...
        try {
            ...
            await this.ipfs.pubsub.subscribe(this.SYNC_TOPIC, (msg) => {
                this.handlePubsubMessage(msg);
            });
            // Request index from other nodes
            await this.ipfs.pubsub.publish(this.SYNC_TOPIC, Buffer.from(JSON.stringify({
                type: 'sync-request',
     ...
     */
    async sendIndexSnapshot(requesterId) {
        try {
            const snapshot = {
                type: 'index-snapshot',
                nodeId: this.nodeId,
            ...
            await this.ipfs.pubsub.publish(this.SYNC_TOPIC, Buffer.from(JSON.stringify(snapshot)));
